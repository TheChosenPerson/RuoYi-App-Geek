...
| send      | 发送信息，当uuid不为空且不为false时，当收到<br />携带相同uuid的消息时触发回调函数，只触发一次。<br />否则由默认处理函数处理。 | msg消息内容，会被处理成json字符串<br />uuid唯一标识符，可以手动传入，也可以设置为true<br />当为true时会自动生成一个uuid并添加到msg中。 |
